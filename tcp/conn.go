...
func getBufferFromPool(targetSize int) []byte {
	...
	} else if targetSize <= 1 >> 11 {
		buf = bufferPool2K.Get().([]byte)
	} else if targetSize <= 1 >> 12 {
...
//A conn represent the server side of an tcp connection
type Conn struct {
	sync.RWMutex
	Opts        *Options
	RawConn     net.Conn
...
func NewConn(opts *Options) *Conn {
	...
	if opts.SendBufSize <= 0 {
		logger.Logf(Warn, "Invalid Opts.SendBufListLen : %v, use DefaultRecvBufSize instead", opts.SendBufSize)
		opts.SendBufSize = DefaultSendBufSize
	}
	c := &Conn {
		Opts :opts,
		sendBufList : make(chan []byte, opts.SendBufSize),
		...
		if err != nil {
			if nerr, ok := err.(net.Error); ok {
				if nerr.Timeout() {
					...
					logger.Logf(Error,"XTCP -  Conn[%v] error : %v; retrying in %v", c.RawConn.RemoteAddr(), err, tempDelay)
